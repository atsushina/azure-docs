...
| `WEBSITE_HTTPLOGGING_RETENTION_DAYS` | Retention period in days of web server logs for a Windows native and application logs for Linux and Custom Container, if web server logs are enabled for a Windows native or application logs are enabled for a Linux or Custom Container app. | `10` |
